...
### WP2 Authentication
For Mastercard Developers APIs implement mTLS / OAuth signing. Keep authentication replaceable for other Mastercard APIs that may use other VDP mechanisms.
...
- Mastercard HTTP/error mapping

### WP5 Idempotency
Generate and persist `correlatnId`.
For Mastercard Developers APIs, preserve the same ID for a safe retry of the same request. Implement retention consistent with the product rules.
...
- operation
- correlation ID
- Mastercard request/reference identifiers
- HTTP status
- latency
