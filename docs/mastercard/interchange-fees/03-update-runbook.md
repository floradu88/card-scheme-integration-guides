...
Ingest a Mastercard interchange fee update for one or more countries, validate it, simulate impact, and activate it with rollback.

## Preconditions

- Access to the official Mastercard schedule / bulletin for the target countries (public hubs and/or Mastercard Connect).
...
1. Create `mastercard-{CC}-{region}-fees` version `X.Y.Z` (bump minor for rate changes, major for structural rule changes).
...
4. Validate against [`schemas/mastercard-country-fee-package.schema.json`](schemas/mastercard-country-fee-package.schema.json).
...
|------|------|------------|
| Cycle update | Scheduled Mastercard regional publication | Minor/major bump, normal simulation window |
