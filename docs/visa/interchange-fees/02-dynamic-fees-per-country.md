...
| `package_id` | Stable ID: `visa-{country}-{region}-fees` |
...
1. Visa publishes a new fee / interchange PDF or bulletin for that market;
...
2. Load active Visa CountryPackage for merchant_country at event_time
...
| Layer | Owns |
|-------|------|
| Network country package | Published Visa schedules for that country / relation |
...
| Actual interchange from settlement | Ground truth for reconciliation (Option E) |
...
Use `network: VISA` and include `merchant_country` / `issuer_country` conditions in rules so multi-country catalogs remain queryable in one engine.
